...
class HotwordRecogniser(object):
    ...
    def hotword_commands_detected(self, command):
        ...
        if "start" != command and "wait" != command:
            self.speech_recognition_client("start", command)
    ...
    def speech_recognition_client(self, action, command):
        ...
        try:
            ...
            response = speech_recognition(action, command)
        except rospy.ServiceException, e:
            print("Speech recog " + action + " service call failed: %s" %e)
    ...
    def stop_recogniser(self):
        ...
        for param in [
                ...
                self._hotword_bring_param, self._hotword_follow_param, 
                self._speech_recognition_srv_param, 
                self._hotword_move_param, self._hotword_say_param,
