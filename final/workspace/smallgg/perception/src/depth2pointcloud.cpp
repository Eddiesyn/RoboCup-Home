...
private:
    ...
    void BoxCallback(const all_msgs::BoxConstPtr& bx);
...
public:
	Depth2PointCloud(ros::NodeHandle nh, float distance) : nh_(nh), priv_nh_("~")
	{
        ...
        sub_box = nh_.subscribe("/God_watcher/Box", 10, &Depth2PointCloud::BoxCallback, this);
        ...
        pub_pt = nh_.advertise<geometry_msgs::PointStamped>("/God_watcher/PersonPoint3D", 10);
...
void Depth2PointCloud::BoxCallback(const all_msgs::BoxConstPtr& bx)
{
    xmin = bx->x;
    ymin = bx->y;
    height = bx->height;
...
int main(int argc, char** argv)
{
	...
	ros::NodeHandle nh;
	Depth2PointCloud depth2pointcloud(nh, 2.7);
