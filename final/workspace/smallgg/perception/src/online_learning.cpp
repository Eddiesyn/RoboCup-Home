...
string negativePath = "/home/atHomeSS18/test_workspace/src/train_data/negative/";
string positivePath = "/home/atHomeSS18/test_workspace/src/train_data/operator/";
// string savePath = "";
bool start = false;
...
private:
	...
	bool PredCallback(all_msgs::send_flags::Request &req, all_msgs::send_flags::Response &res);
	void PredictCallback(const sensor_msgs::ImageConstPtr& _img);
...
public:
	Online_learning(ros::NodeHandle nh) : nh_(nh), priv_nh_(nh), it_(nh)
	{
		...
		predict_begin = nh_.advertiseService("/online_learning/predict", &Online_learning::PredCallback, this);
...
bool Online_learning::PredCallback(all_msgs::send_flags::Request &req, all_msgs::send_flags::Response &res)
{
	if(req.flag)
	{
		pred_start = true;
